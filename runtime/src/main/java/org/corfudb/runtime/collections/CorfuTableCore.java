...
@Slf4j
@CorfuObject
public class CorfuTableCore<K ,V, F extends Enum<F> & CorfuTable.IndexSpecification, I>
    ...
    public CorfuTableCore(Class<F> indexFunctionEnumClass) {
    ...
    public @Nonnull <P> Collection<P> getByIndex(@Nonnull F indexFunction,
                                @Nonnull CorfuTable.ProjectionFunction<K, V, I, P> projectionFunction,
                                I index) {
    ...
    @DontInstrument
    protected V undoPutRecord(CorfuTableCore<K, V, F, I> table, K key, V value) {
        return table.mainMap.get(key);
    }

    @DontInstrument
    protected void undoPut(CorfuTableCore<K, V, F, I> table, V undoRecord, K key, V value) {
    ...
    @DontInstrument
    @SuppressWarnings("unchecked")
    Map<K,V> undoPutAllRecord(CorfuTableCore<K, V, F, I> previousState,
     ...
     */
    @DontInstrument
     void undoPutAll(CorfuTableCore<K, V, F, I> table, Map<K,V> undoRecord,
                     Map<? extends K, ? extends V> m) {
    ...
    @DontInstrument
    protected V undoRemoveRecord(CorfuTableCore<K, V, F, I> table, K key) {
        return table.mainMap.get(key);
    }

    @DontInstrument
    protected void undoRemove(CorfuTableCore<K, V, F, I> table, V undoRecord, K key) {
        if (undoRecord == null) {
