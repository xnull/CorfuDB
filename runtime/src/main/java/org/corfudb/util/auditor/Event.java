...
public class Event implements Serializable {
    ...
    private Event(Type eventType, String mapId, String threadId,
                  String txId, String txType, String key, String valueSize) {
    ...
    public static Event newEventInstance(String typeString, String mapId, String threadId,
                                         String txId, String txType, Object... args) {
        Type type = Type.fromString(typeString);
        Event event;

        switch (type) {
            case OPCLEAR:
            event = createEvent(type, mapId, threadId, txId, txType, NOT_AVAILABLE, null);
            break;
            case OPBLINDPUT:
                event = createEvent(type, mapId, threadId, txId, txType, String.valueOf(args[0]), args[1]);
                break;
            case OPCONTAINSKEY:
                event = createEvent(type, mapId, threadId, txId, txType, String.valueOf(args[0]), null);
                break;
            case OPCONTAINSVALUE:
                event = createEvent(type, mapId, threadId, txId, txType, NOT_AVAILABLE, args[0]);
                break;
            case OPENTRYSET:
                event = createEvent(type, mapId, threadId, txId, txType, NOT_AVAILABLE, null);
                break;
            ...
            default:
                event = createEvent(Type.UNKNOWN, mapId, threadId, txId, txType, NOT_AVAILABLE, null);
                break;
    ...
    private static Event createEvent(Type eventType, String mapIds, String threadId, String txId,
                                     String txType, String key, Object valueObject) {
        return new Event(
                eventType,
                mapIds,
                threadId,
